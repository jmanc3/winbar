...
paint_option(AppClient *, cairo_t *cr, Container *container) {
    ...
    if (container->state.mouse_hovering || keep_open ||
...
//    container->wanted_bounds.h = actual_true_height(container);
    client_create_animation(app, client, &container->wanted_bounds.h, 0, 250, getEasingFunction(EaseOutElastic),
                            actual_true_height(container), true);
...
option_leaves(AppClient *client, cairo_t *, Container *container) {
    ...
    for (auto *child : container->children) {
        ...
        if (data->text.find("...") != std::string::npos) {
            keep_open = true;
        } else if (data->text.find("Cancel") != std::string::npos) {
            keep_open = true;
        }
    }
...
//                parent->when_paint = paint_option_name;
                ...
                if (has_options(wanted_device->icon)) {
                    std::string path;
                ...
                if (wanted_device->paired) {
                    auto connect = parent->child(::vbox, FILL_SPACE, button_height * config->dpi);
                    ...
                    auto forget = parent->child(::vbox, FILL_SPACE, button_height * config->dpi);
                    ...
                    auto pair = parent->child(::vbox, FILL_SPACE, button_height * config->dpi);
