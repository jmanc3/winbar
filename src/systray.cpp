...
start_systray()
{
    ...
    handler->event_handler = systray_event_handler;
    handler->target_window = systray->window;
...
open_systray()
{
    ...
    auto* handler = new Handler;
    handler->event_handler = display_event_handler;
    handler->target_window = display->window;
    app->handlers.push_back(handler);
